...
function openWindow(id) {
  ...
  if (id !== 'themeBuilderWindow') requestAnimationFrame(() => { injectWindowFx(id); pxlReInject(); });
}
...
let fxIntervals = [];
let fxRAFs = [];
let fxGen = 0; // increment on clearThemeFx to kill all running RAF loops
...
function clearThemeFx() {
  fxGen++; // kill all running RAF loops
...
function buildOverlaySettings(id) {
  ...
  if (!defs.length) {
    // Always show 2 placeholder rows so card height is consistent
    body.innerHTML =
...
/* ── Preview-only overlay (theme builder only, never touches real layers) ── */

function injectWindowFx(windowId) {
  const win = document.getElementById(windowId);
  if (!win) return;
  removeWindowFx(windowId); // clear any existing
  const cfg = getOverlayCfg();
  const id = cfg.overlay || 'none';
  if (id === 'none') return;
  const c = cfg[id] || {};
  const layer = c.layer || 'under';
  // create back and fore layers inside the window
  ...
  win.style.position = 'fixed'; // ensure absolute children position correctly
  win.appendChild(wb);
  win.appendChild(wf);
  // apply overlay into window fx divs
...
function applyOverlayInWindow(id, c, backEl, foreEl) {
  ...
  if (id === 'grn') {
    ...
    function d() { if(win._winFxGen!==myGen)return;
    ...
    const colMap={green:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(80deg) brightness(1.1)',amber:'brightness(0.75) contrast(1.3) saturate(0) sepia(1)',blue:'brightness(0.82) contrast(1.35) saturate(0) sepia(1) hue-rotate(165deg) saturate(2)',white:'brightness(0.9) contrast(1.15) saturate(0)',red:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(320deg)',purple:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(250deg)',cyan:'brightness(0.8) contrast(1.35) saturate(0) sepia(1) hue-rotate(130deg) saturate(2.5)',orange:'brightness(0.78) contrast(1.3) saturate(0) sepia(1) hue-rotate(355deg)'};
    ...
    let wy=0;
    const iv=setInterval(()=>{if(win._winFxGen!==myGen){clearInterval(iv);win.style.filter='';return;}wy=(wy+2)%(win.offsetHeight||800);wave.style.top=wy+'px';},16);
  } else if (id === 'stt') {
    ...
    const body2 = win.querySelector('.data-body') || win;
    const kids = () => Array.from(body2.children).filter(el=>!el.classList.contains('win-fx-back')&&!el.classList.contains('win-fx-fore'));
    const delay=()=>Math.round({slow:600,med:280,fast:80}[c.speed||'med']+Math.random()*300);
    const maxSh={low:10,med:22,high:40}[c.intensity||'med'];
    ...
    const ps={small:3,med:5,large:8}[c.size||'med'];const op={light:.18,med:.28,heavy:.4}[c.intensity||'med'];
    const cv=mkWinC(target,op,null);const ctx=cv.getContext('2d');
    if(!cv.width||!cv.height)return;
    for(let y=0;y<cv.height;y+=ps)for(let x=0;x<cv.width;x+=ps){const base=rgbs[Math.floor(Math.random()*rgbs.length)];const dark=Math.random()<.38;const f=dark?(.15+Math.random()*.25):(.55+Math.random()*.45);const n=v=>Math.max(0,Math.min(255,Math.round(v*f+(Math.random()-.5)*20)));ctx.fillStyle=`rgb(${n(base[0])},${n(base[1])},${n(base[2])})`;ctx.fillRect(x,y,ps,ps);}
  } else if (id === 'vgn') {
...
function applyOverlayToPreview(id) {
  // Stop any existing preview animation loop
  ...
  if (id === 'grn') {
    const op = {low:.12,med:.22,high:.38}[c.intensity||'med'];
    const ps = {fine:1,med:2,coarse:4}[c.size||'med'];
    const cv = mkPreviewC('fx-back', op, null);
    const ctx = cv.getContext('2d');
    function drawGrn() {
      if (window._previewGen !== myPreviewGen) return;
      const img = ctx.createImageData(cv.width, cv.height);
      for (let y=0;y<cv.height;y+=ps) for (let x=0;x<cv.width;x+=ps) {
        const v=Math.random()*255;
        for (let dy=0;dy<ps;dy++) for (let dx=0;dx<ps;dx++) {
          const i=((y+dy)*cv.width+(x+dx))*4;
          if(i<img.data.length-3){img.data[i]=img.data[i+1]=img.data[i+2]=v;img.data[i+3]=Math.random()*65;}
        }
      }
      ctx.putImageData(img,0,0);
      requestAnimationFrame(drawGrn);
    }
    drawGrn();
  }

  else if (id === 'crt') {
    const colMap = {green:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(80deg) brightness(1.1)',amber:'brightness(0.75) contrast(1.3) saturate(0) sepia(1)',blue:'brightness(0.82) contrast(1.35) saturate(0) sepia(1) hue-rotate(165deg) saturate(2) brightness(0.9)',white:'brightness(0.9) contrast(1.15) saturate(0)',red:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(320deg) saturate(1.5)',purple:'brightness(0.78) contrast(1.4) saturate(0) sepia(1) hue-rotate(250deg) saturate(1.8)',cyan:'brightness(0.8) contrast(1.35) saturate(0) sepia(1) hue-rotate(130deg) saturate(2.5)',orange:'brightness(0.78) contrast(1.3) saturate(0) sepia(1) hue-rotate(355deg) saturate(1.8)'};
    const tintMap = {green:'rgba(0,255,60,0.10)',amber:'rgba(255,160,0,0.12)',blue:'rgba(0,140,255,0.14)',white:'rgba(255,255,255,0.06)',red:'rgba(255,40,0,0.14)',purple:'rgba(160,0,255,0.14)',cyan:'rgba(0,255,220,0.14)',orange:'rgba(255,120,0,0.14)'};
    const col = c.color||'green';
    const sp = {light:4,med:3,heavy:2}[c.lines||'med'];
    if (demoEl) demoEl.style.filter = colMap[col];
    const tgtEl = layer==='over' ? pfore : pback;
    if (tgtEl) {
      const sl=document.createElement('div');
      sl.style.cssText=`position:absolute;inset:0;background:repeating-linear-gradient(to bottom,transparent 0,transparent ${sp}px,rgba(0,0,0,0.45) ${sp}px,rgba(0,0,0,0.45) ${sp+1}px);`;
      tgtEl.appendChild(sl);
    }
    if (pfore) {
      const vg=document.createElement('div'); vg.style.cssText='position:absolute;inset:0;background:radial-gradient(ellipse at center,transparent 38%,rgba(0,0,0,0.92) 100%);'; pfore.appendChild(vg);
      ...
      wave.style.cssText=`position:absolute;left:0;right:0;height:30px;background:linear-gradient(to bottom,transparent,${tintMap[col]},transparent);`;
      pfore.appendChild(wave);
      let wy=0;
      function animCRTWave() {
        if (window._previewGen !== myPreviewGen) return;
        const h2 = pfore.offsetHeight || 200;
        wy = (wy + 2) % h2; wave.style.top = wy + 'px';
        requestAnimationFrame(animCRTWave);
      }
      animCRTWave();
    }
  }
    ...
    function drawStt() {
      if (window._previewGen !== myPreviewGen) return;
      const img=ctx.createImageData(cv.width,cv.height);
      for(let i=0;i<img.data.length;i+=4){const v=Math.random()*255;img.data[i]=img.data[i+1]=img.data[i+2]=v;img.data[i+3]=175;}
      ctx.putImageData(img,0,0);
      requestAnimationFrame(drawStt);
    }
    drawStt();
  }
    ...
    const delay = ()=>Math.round({slow:600,med:280,fast:80}[c.speed||'med']+Math.random()*300);
    const maxSh = {low:8,med:18,high:30}[c.intensity||'med'];
    function spawnGlc() {
      if (window._previewGen !== myPreviewGen) return;
      const r = Math.random();
      const els = kids();
      if (r < 0.5 && els.length) {
        ...
        const sh=(Math.random()-.5)*maxSh*2;
        el.style.transform=`translateX(${sh}px)`;
        setTimeout(()=>{if(el)el.style.transform='';},20+Math.random()*60);
      } else if (els.length) {
        els.forEach(el=>{el.style.transform=`translateX(${(Math.random()-.5)*maxSh}px)`;});
        setTimeout(()=>els.forEach(el=>{if(el)el.style.transform='';}),15+Math.random()*40);
      }
    ...
    const spd={slow:.12,med:.35,fast:.85}[c.speed||'med'];
    const n={sparse:20,med:40,dense:70}[c.density||'med'];
    [{target:'fx-back',n,minSz:.5,maxSz:2,sp:spd*.3,op:.25},
     {target:'fx-fore',n:Math.round(n*.2),minSz:3,maxSz:6,sp:spd*1.2,op:.4}
    ].forEach(({target,n:cn,minSz,maxSz,sp,op})=>{
    ...
    const target = layer === 'over' ? 'fx-fore' : 'fx-back';
    const cv=mkPreviewC(target,op,null);
    cv.style.mixBlendMode='overlay';
    const ctx=cv.getContext('2d');
    if(!cv.width||!cv.height)return;
    for(let y=0;y<cv.height;y+=ps)for(let x=0;x<cv.width;x+=ps){
      const v=Math.floor(Math.random()*256);
      ctx.fillStyle=`rgb(${v},${v},${v})`;ctx.fillRect(x,y,ps,ps);
    }
  }
...
function pxlReInject() {
  // Find existing BIG canvas from fx-back, reuse its pixel data
  const fxb = document.getElementById('fx-back');
  const existingBig = fxb && fxb.querySelector('canvas');
  if (!existingBig) { applyOverlay(); return; } // full redraw if no source
  const cfg = getOverlayCfg();
  const c = cfg.pxl || {};
  const ps = {small:3,med:5,large:8}[c.size||'med'];
  const op = {light:.15,med:.3,heavy:.55}[c.intensity||'med'];
  // Remove only stale pxl-inner canvases (whose parent no longer has them as child)
  document.querySelectorAll('.pxl-inner').forEach(el => el.remove());
  // Re-inject using existing BIG as source
...
function pxlInjectRoot(root, BIG, ps, op) {
  // Walk every element inside root, inject canvas where element has a visible background
  const all = root.querySelectorAll('*');
  all.forEach(el => {
    if (el.classList.contains('pxl-inner')) return; // skip existing
    const cs = getComputedStyle(el);
    const bg = cs.backgroundColor;
    if (!bg || bg === 'rgba(0, 0, 0, 0)' || bg === 'transparent') return;
    // Skip elements with no dimensions
    const r = el.getBoundingClientRect();
    if (!r.width || !r.height || r.width < 2 || r.height < 2) return;
    // Skip if already has pxl-inner child
    if (el.querySelector(':scope > .pxl-inner')) return;

    el.style.position = el.style.position || 'relative';
    if (!el.style.zIndex || el.style.zIndex === 'auto' || el.style.zIndex === '') {
      el.style.zIndex = '0';
    }
    ...
    cv.style.cssText = `position:absolute;inset:0;width:100%;height:100%;opacity:${op};mix-blend-mode:overlay;pointer-events:none;z-index:-1;`;
    try {
      cv.getContext('2d').drawImage(BIG, r.left, r.top, r.width, r.height, 0, 0, r.width, r.height);
    } catch(e) { return; }
...
function getOverlayFns() { return {
  ...
  grn: (c) => {
    ...
    const ctx = cv.getContext('2d');
    const myGen = fxGen;
    function d() { if (fxGen !== myGen) return;
      const img = ctx.createImageData(cv.width, cv.height);
      for (let y=0;y<cv.height;y+=ps) for (let x=0;x<cv.width;x+=ps) { const v=Math.random()*255; for(let dy=0;dy<ps;dy++) for(let dx=0;dx<ps;dx++){const i=((y+dy)*cv.width+(x+dx))*4;if(i<img.data.length-3){img.data[i]=img.data[i+1]=img.data[i+2]=v;img.data[i+3]=Math.random()*65;}}}
      ctx.putImageData(img,0,0); requestAnimationFrame(d); }
    d();
...
fxIntervals.push(setInterval(() => { if(fxGen!==crtGen)return; wy=(wy+2)%wrapH; wave.style.top=wy+'px'; }, 16));
  ...
  pxl: (c) => {
    ...
    // Generate one big greyscale source — random light/dark blocks
    // overlay blend mode lightens/darkens the underlying color without changing its hue
    const BIG = document.createElement('canvas');
    BIG.width = window.screen.width;
    BIG.height = window.screen.height;
    const bctx = BIG.getContext('2d');
    for (let y = 0; y < BIG.height; y += ps) {
      for (let x = 0; x < BIG.width; x += ps) {
        const v = Math.floor(Math.random() * 256);
        bctx.fillStyle = `rgb(${v},${v},${v})`;
        bctx.fillRect(x, y, ps, ps);
      }
    }

    // 1. Background canvas at z:1 — below .app (z:2)
    const fxb = document.getElementById('fx-back');
    if (fxb) {
      const bgCv = document.createElement('canvas');
      bgCv.width = BIG.width; bgCv.height = BIG.height;
      bgCv.style.cssText = `position:absolute;top:0;left:0;opacity:${op};mix-blend-mode:overlay;`;
      bgCv.getContext('2d').drawImage(BIG, 0, 0);
      fxb.appendChild(bgCv);
    }

    // 2. Inject per-element canvases into every visible element with a background
    // Targets the entire app + any open windows
    const roots = [document.getElementById('mainApp')];
    document.querySelectorAll('.data-window.open').forEach(w => roots.push(w));
    roots.forEach(root => {
      if (!root) return;
      pxlInjectRoot(root, BIG, ps, op);
    });
  },
...
function renderJobs() {
  ...
  if (typeof renderHistory === 'function') { buildHistory(); renderHistory(); }
  // Re-inject pixel overlay into newly rendered elements
  requestAnimationFrame(() => {
    const cfg = getOverlayCfg();
    if (cfg.overlay === 'pxl') pxlReInject();
  });
}
...
function openJobWindow(job) {
  ...
  requestAnimationFrame(() => { injectWindowFx('jobWindow'); pxlReInject(); });
}
